...
- recursive entity tagging - wikidata - has subclass - has subclass etc. - each has 0.x^n weight
- taj mahal na wikidata - neni jen mausoleum
- entity wikidata - may misrepresent - e.g. the lesser known sun tzu - https://www.wikidata.org/wiki/Q2634383
------------------------
...
Administrativní registr ekonomických subjektů, pro mnohé zdroj informací o firmách, pro vývojáře až nepřátelský systém pro datovou extrakci. Po letech 

co znamená 6000 requestů za den při milionu záznamů

chce to RES, jinak nemáme info o ministerstvech, nemáme právní formy atd. (které jsou ve specifikaci, ale do VREO se nedostaly)
...
- proč nový ARES vydali (Bláha, Kubáň)
- o srazu na MFČR, kde paradoxně všichni chtěli dump ARESu, ale přitom ho vsichni měli
- Jak mfcr dumpuje vlastni dataset
- dump pomůže MFČR, protože lidi nebudou bombardovat ARES requestama (ale vzhledem k neúplnosti to budou dělat nadále)
- co znamená celý ARES
- jaká je frekvence atd.
- další datasety - zejm. plné IČO - dřív to měl člověk z dumpu ($$$, ČSÚ?), od staťáku ($), ze státní pokladny (kde bylo 4x víc subjektů než v OR), ze seznamu datovek, nebo sledováním změň v rejstříku (čímž nezískal zaniklý ani dlouho neaktualizovaný)
- struktura XML je teď jednodušší na operaci - zejm. angažované osoby

- dotazy nad celkem - kdo vlastní koho
	- příklady - vezmeme strukturální fondy a dohledáme lidi napojené na dotace (do n-tého kolene klidně)

- limitace - nemáme historii (přidat odkaz do ARES pro rozsah=2), takže nevidíme, kdo byl haguess, kde byla sídla atd. (ukázat historii v XML službách), ale známe bývalé angažované osoby


https://www.czso.cz/csu/res/o_registru_res


blaha
Důležitost ARES podtrhuje i fakt, že byl loni zařazen na seznam 11 zdrojů dat, které musí úřady povinně otevřít ve formě open dat. Součástí Ministerstva financí je i oddělení Open Dat (odbor Centrální harmonizační jednotka), které je za open data resortu zodpovědné a které má jasné pravomoci v této oblasti vůčí zbytku MF ČR.
...
------------------------
title: lead scoring using wikidata

# upravit tam int(emp) na int(float(emp)), protoze nekdo nekde vlozil 2.400 - opravit to ve wikidatech?
# ValueError: invalid literal for int() with base 10: '+2.400'
...
# Testing:
# curl $url | head -c 10000 > sample.json.gz
# 
# Prod:
# curl $ url | split -n 10000 ... paralelise
...
fn = 'latest-all.json.gz'

with gzip.open(fn, 'rt') as gf, open('websites.csv', 'w') as fw:
    cw = csv.writer(fw)
    cw.writerow(['id', 'label', 'website', 'employees'])
    for j, ln in enumerate(gf):
        ...
        dt = json.loads(ln[:-2]) # trim ',\n'
        eid = dt['id']
        label = dt['labels'].get('en', {}).get('value')

        # we need websites and employees
        if not ('P856' in dt['claims'] and 'P1128' in dt['claims']):
            continue

        ws = [j['mainsnak']['datavalue']['value'] for j in dt['claims']['P856']]

        # employees but not when it's only a partial value (P518)
        emp = [int(j['mainsnak']['datavalue']['value']['amount']) for j in dt['claims']['P1128'] if not 'P518' in j.get('qualifiers', {})]
        emp = max(emp or [None]) # TODO: this could be better, good enough here
        
        for w in ws:
            nl = urlparse(w).netloc
            nl = nl[4:] if nl.startswith('www.') else nl
            cw.writerow([eid, label, nl, str(emp)])
...
------------------------
title: Custom containers in Python

Try advent of code and use custom classes with getitem, setitem, repr, iterators
...
------------------------
title: PyData talk about losing data in Spark

Losing lots of data very quickly

Takeaways - correctness always trumps performance
Explicit is better than implicit (dateutil)
Fail early, fail often (armbrust quote)

Low performance data loss - dateutil

The job of dataeng is to move data *reliably* - the end user doesnt care if it’s Spark or Cobol, they want their numbers to line up. 

Start with a quoted csv and ask for a count of a given value

Lessons:
1. KNOW YOUR TOOLS
2. It's hard to know your tools, if they are super complex
3. Know why Spark defaults to line-based reading - it's much faster, because you can paralelise it

It's quite easy to know 4180 + the builtin csv package in Python


Also - Spark is fast, it can lose data, so let's call it "Losing data really fast"

But we'll also talk about hijacking analyses

It's like TRUNCATE (CASCADE), it's really fast and it can destroy your database really quickly.

it's all about multiline and escape='"'

but you don't use quotes? What about a 2.5" disk?

not just spark - there was a discussion about sensible defaults in scikit learn - about L2 regularisation

what about pandas? when we put floats and specify dtype=int

TypeError: Cannot cast array from dtype('float64') to dtype('int64') according to the rule 'safe'


make sure to test all code on the latest pandas and pyspark

include JIRA tickets (investigate the rest)

Parquet is not a default, CSV still is
Also pq as incompatibilities pf its own
Show losing data on that

you can not only pass 2019-02-30, but also 25:00:00 as time!!!

dateutil is great, but it has the same problem as spark - you dont know what its doing


it was wrong, but really fast https://pics.me.me/cleeen-funnies-leeen-funnie-interviewer-it-says-here-you-are-really-25740139.png
------------------------
...
Every 1.0s: sort -r log.txt                                                             opendata: Sat May 11 12:37:22 2019

0.8534781632880022,uyjapixmhtnqslokgzfrwdcveb,2019-05-11 12:35:10.385038
0.8534361522333411,uyjapwxmhtnqslokgzfridcveb,2019-05-11 12:34:43.730225
0.8529486708961032,uyjapwxmhtnqsdokgzfrilcveb,2019-05-11 12:29:28.145618
0.8529429014390268,uyjapwxmhtnqsodkgzfrilcveb,2019-05-11 12:17:08.230288
0.8520310765198101,uydapwxmhtnqsojkgzfrilcveb,2019-05-11 12:22:07.205905
0.8512269682411281,uyjaiwxmhtnqsodkgzfrplcveb,2019-05-11 12:24:42.971040
0.8510579800939702,uyjapwxmqtnhsodkgzfrilcveb,2019-05-11 12:21:23.631169
0.8502529078187472,uyjapwxmhtnqsodkgzfrlicveb,2019-05-11 12:02:47.874138
0.8482618641265682,uyjapwxmhtnqsdokgzfrlicveb,2019-05-11 11:58:36.808219
0.8464843007496279,uydapwxmhtnqsojkgzfrlicveb,2019-05-11 11:56:39.673123
0.8459339765378661,uyjapwxmhtnsqodkgzfrilcveb,2019-05-11 12:28:54.779006
0.8458039641617907,uyjapwxmhtnqsodkgzfriclveb,2019-05-11 12:27:05.266669
0.8444668111462574,uyjapwxmhtnqsokdgzfrlicveb,2019-05-11 12:05:20.127367
0.8424309626717439,uyjapixmhqntslokgzfrwdcveb,2019-05-11 12:35:19.925657
0.8420154217166026,uydapwxmhtnqsolkgzfrjicveb,2019-05-11 11:57:34.992034
0.840111753735119,uyjapixmhtnqsokdgzfrwlcveb,2019-05-11 12:29:23.228323
0.8395494859369559,uydapwxmhtqnsojkgzfrlicveb,2019-05-11 11:57:54.732383
0.8362629634456387,uyjapixmhtnqsdokgzfrwlcveb,2019-05-11 12:19:35.668189
0.8355432759482186,uyjapwxmhtnisodkgzfrlqcveb,2019-05-11 12:05:26.183178
0.8345273397577885,uyjapixmhtnqsodkgzfrlwcveb,2019-05-11 12:08:59.625597
0.8344211842764502,lyjapwxmhtnqsodkgzfriucveb,2019-05-11 12:24:33.545041
0.8329701254283253,uydapwxmhtnqsojkgzfrcilveb,2019-05-11 11:56:23.238392
0.8318758789566237,uyjapwxmhtnqsoikgzfrdlcveb,2019-05-11 12:26:07.577171
------------------------
